...
// ── Goertzel pre-filter (1 kHz BW at 8 kHz) ─────────────────
static constexpr int CW_FIR_TAPS = 65;
static constexpr float CW_FIR_CUTOFF_HZ = 500.0f;
...
// ── APF (now handled by QMX via CAT — local filter commented out) ──
...
static constexpr int GOERTZEL_BINS = 1001;     // ±500 Hz at 1 Hz resolution
