...
void dsp::init(int sample_rate, int fft_size)
{
    ...
    memset(g_input_buf[0], 0, fft_size * sizeof(float));
    memset(g_input_buf[1], 0, fft_size * sizeof(float));
    g_write_buf    = 0;
    g_write_pos    = 0;
    g_ready_buf    = -1;
...
void dsp::pushSamples(const float *samples, int count)
{
    for (int i = 0; i < count; i++) {
        ...
        if (g_write_pos >= g_fft_size) {
            // Full memory barrier: ensure all buffer writes are visible
            // to other cores before signaling the buffer is ready
            __sync_synchronize();
...
bool dsp::processIfReady()
{
    ...
    // Full memory barrier: ensure we see the buffer data written by pushSamples
    __sync_synchronize();
